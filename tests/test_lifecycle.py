...
def test_cleanup_sends_sigterm_and_returns_reclaimed_pids(monkeypatch):
    monkeypatch.setattr(lifecycle, "find_managed_orphan_ss_local", lambda: [1234])
    sent = []
    ...
    assert lifecycle.cleanup_managed_orphan_ss_local(timeout=0) == [1234]
    assert sent == [(1234, signal.SIGTERM)]


def test_cleanup_escalates_to_sigkill_when_process_survives(monkeypatch):
    monkeypatch.setattr(lifecycle, "find_managed_orphan_ss_local", lambda: [4321])
    sent = []
    monkeypatch.setattr(lifecycle.os, "kill", lambda pid, sig: sent.append((pid, sig)))
    ...
    assert lifecycle.cleanup_managed_orphan_ss_local(timeout=0) == [4321]
